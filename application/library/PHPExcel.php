...
class PHPExcel
{
    ...
    */
    ...
    /**
     *
     $header = ["名字","性别","年龄","出生日期","电话号码"];

    $data = [
        ['王城', '男', '18', '1997-03-13', '18948348924'],
        ['李飞虹', '男', '21', '1994-06-13', '159481838924'],
        ['王芸', '女', '18', '1997-03-13', '18648313924'],
        ['郭瑞', '男', '17', '1998-04-13', '15543248924'],
        ['李晓霞', '女', '19', '1996-06-13', '18748348924']
    );
     ...
     * @throws
     */
    public function OutPut2Excel(PHPExcel $objPHPExcel, array $header, array $datas, string $filename, array $dataTypeArray=[])
    {
        $sheetColNameArr = $this->sheetColNameArr;
        $headerObj = $objPHPExcel->setActiveSheetIndex(0);
        foreach ($header as $key=>$item)
        {
            $headerObj = $headerObj->setCellValue($sheetColNameArr[$key]."1",$item);
            $objPHPExcel->getActiveSheet()->getColumnDimension($sheetColNameArr[$key])->setWidth(18);
            $objPHPExcel->getActiveSheet()
                ->getStyle($sheetColNameArr[$key]."1")
        ...
        foreach($datas as $data){
            ...
            {
                $objPHPExcel->getActiveSheet()
                    ->setCellValueExplicit($sheetColNameArr[$k] . $i, $v, ( isset($dataTypeArray[$k]) && $dataTypeArray[$k]=='numeric' ) ? PHPExcel_Cell_DataType::TYPE_NUMERIC : PHPExcel_Cell_DataType::TYPE_STRING )
                    ->getStyle($sheetColNameArr[$k].$i)
                    ->getAlignment()
                    ->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            }
            $i++;
