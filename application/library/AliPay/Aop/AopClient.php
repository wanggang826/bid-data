...
class AopClient {
	...
	protected function curl(string $url, array $postFields = []) {
		$ch = curl_init();
    ...
    {
        \Log::Error(
            'Alipay  apiName:{apiName} requestUrl:{requestUrl} errorCode:{errorCode} responseTxt{responseTxt}',
            [
                '{apiName}'     => $apiName,
	...
	*/
	...
	public function buildUrl($request, $authToken = null, $appInfoAuthtoken = null, $isToH5=false)
    {
        $this->setupCharsets($request);
        ...
        //签名
        $sign = $this->generateSign(array_merge($apiParams, $sysParams), $this->signType);
    ...
    {
        $info = $this->buildUrl($request, $authToken, $appInfoAuthtoken);
        $requestUrl = $info[0];
        $apiParams  = $info[1];
        $sysParams  = $info[2];
	 ...
	 **/
	public function rsaCheckV1(array $params, string $rsaPublicKeyFilePath, string $signType='RSA')
    {
		$sign = $params['sign'];
        $params['sign']      = null;
	...
	public function rsaCheckV2(array $params, string $rsaPublicKeyFilePath, string $signType='RSA')
    {
		$sign = $params['sign'];
		$params['sign'] = null;
		return $this->verify($this->getSignContent($params), $sign, $rsaPublicKeyFilePath, $signType);
	...
	public function rsaEncrypt($data, $rsaPublicKeyPem, $charset)
    {
		...
		{
			if (!openssl_public_encrypt($block, $charText, $res))
			{
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$encodes[] = $charText;
		}
        $charText = implode(",", $encodes);

		return $charText;
	}

	public function rsaDecrypt(string $data, string $rsaPrivateKeyPem)
    {
		//读取私钥文件
	...
	function parserXMLSignData($request, $responseContent) : \SignData
    {
		$signData = new SignData();
	...
	function parserXMLSource(string $responseContent, string $nodeName, int $nodeIndex) : string
    {
	...
	function parserXMLSign($responseContent) : string
    {
